...
```{r}
ggplot(raw[!is.na(raw$y),], aes(x = y, fill = y)) +
  geom_bar(stat='count') +
...
```
##### 3.3. Feature Selection (designed methods) 
```{r}
### optional ###
...
RF.model <- mining(x = y ~ .
                   ...
                   , task = 'prob'  #'class','reg','default'
                   , method = c('holdoutorder', 1000)
                   , search=list(search = mparheuristic("randomForest", n=5)# 10x5 needs 3100s to run.
                                 #, method = c('holdoutrol',100) 
                                 , method = c('kfoldorder',2)
                                 , metric = "AUC" 
                   )
...
lines(m3,META.roc[["sensitivities"]],col="pink",lwd=5)
lines(m4,CLU.roc[["sensitivities"]],col="orange",lwd=3)

lines(NN.metric[[1]][["roc"]][["roc"]][,2]~NN.metric[[1]][["roc"]][["roc"]][,1],col="blue",lwd=1)
lines(RF.metric[[1]][["roc"]][["roc"]][,2]~RF.metric[[1]][["roc"]][["roc"]][,1],col="purple",pch=0,lwd=1)
...
legend("bottomright"
       ,legend=c("LR","DT","SV","NN","RF","BAGG","BOOST","KNN",'META','META')
