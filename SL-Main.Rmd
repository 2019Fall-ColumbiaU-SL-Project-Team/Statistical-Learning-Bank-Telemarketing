...
NN.model2 <- mining(x = y ~ .
                    ...
                    , search=list(search=mparheuristic("mlp", n= 5), method = c('kfoldrandom',5) , metric="AUC" )
                    #, feature = 'none'
                    #, search = 
                    #, scale = 
                    #, transform = 
...
RF.model <- mining(x = y ~ .
                   ...
                   , task = 'prob'  #'class','reg','default'
                   , method = c('holdoutorder', 1000)
                   , search=list(search = mparheuristic("randomForest", n=1)# 10x5 needs 3100s to run.
                                 #, method = c('holdoutrol',100) 
                                 , method = c('kfoldorder',1)
                                 , metric = "AUC" 
                   )
...
BAGG.model <- mining(x = y ~ .
                     ...
                     , task = 'prob'  #'class','reg','default'
                     , method = c('holdoutorder', 1000)
                     , search=list(search=mparheuristic("bagging", n= 2)
...
KNN.model <- mining(x = y ~ .
                    ...
                    , task = 'prob'  #'class','reg','default'
                    , method = c('holdoutorder', 1000)
                    # , search=list(search=mparheuristic("knn", n= 1)
                    #               , method = c('kfoldrandom',1) 
                    #               , metric="AUC" 
                    #)
                    #, feature = 'none'
                    #, search = 
...
model_selected  <-  c('LR','NN','BOOST','KNN')
...
Weight <-  (AUC_selected-0.5776)/(1-0.5776)
...
colnames(AUC_all)[8] <- 'META'

PROBS_all <- cbind(PROBS, META.probs)
colnames(PROBS_all)[8] <- 'META'
